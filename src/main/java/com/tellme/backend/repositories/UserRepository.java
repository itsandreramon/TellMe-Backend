...
public class UserRepository implements UserDao {
    ...
    public List<User> getAllUsersByQuery(String query, int limit) {
        var searchQuery = userCollection.whereGreaterThanOrEqualTo(Constants.USER_KEY_USERNAME, query).limit(limit);
        ...
        try {
            ...
            if (querySnapshot.get().getDocuments().size() > 0) {
                ...
                return querySnapshot.get().getDocuments().stream()
                        .map(document -> FirebaseUtil.mapDocumentToUser(document))
    ...
    public Optional<Boolean> followUserByUid(String userUid, String userToFollowUid) {
        ...
        var querySnapshot = userCollection
                .whereEqualTo(Constants.USER_KEY_UID, userUid)
                .limit(1)
                .get();
